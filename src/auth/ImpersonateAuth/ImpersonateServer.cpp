...
#include "../auth/ImpersonateAuth/ImpersonateServer.h"
#include "../auth/ImpersonateAuth/ImpersonateHash.h"
...
class ImpersonateServer FB_FINAL :
	public StdPlugin<IServerImpl<ImpersonateServer, CheckStatusWrapper> >
{
public:
	explicit ImpersonateServer(IPluginConfig* p)
		: iParameter(p), authenticated(false), 
		secureDbName(getPool())
	{
	}
...
int ImpersonateServer::authenticate(CheckStatusWrapper* status, IServerBlock* sBlock,
	IWriter* authBlock)
{
	status->init();
	...
	try
	{
		if (!authenticated)
		{
			...
			if (!user)
			{
				HANDSHAKE_DEBUG(fprintf(stderr, "ImpersonateServer (nologin) %d\n", IAuth::AUTH_CONTINUE));
			...
			if (!(data && length))
			{
				HANDSHAKE_DEBUG(fprintf(stderr, "ImpersonateServer (nopw) %d\n", IAuth::AUTH_MORE_DATA));
			...
			if (!found)
			{
				HANDSHAKE_DEBUG(fprintf(stderr, "ImpersonateServer (badlogin) %d\n", IAuth::AUTH_CONTINUE));
				return IAuth::AUTH_CONTINUE;
			}
			...
			string newHash;
			ImpersonateHash::hash(newHash, login, passwordEnc, storedHash);
			if (newHash != storedHash)
			{
				...
				if (!legacyHash)
				{
					HANDSHAKE_DEBUG(fprintf(stderr, "ImpersonateServer (badpw) %d\n", IAuth::AUTH_CONTINUE));
					return IAuth::AUTH_CONTINUE;
				}
			}
			...
			string serverData("ImpersonateServerKey");
			sBlock->putData(status, serverData.length(), serverData.c_str());
			...
			authenticated = true;
			HANDSHAKE_DEBUG(fprintf(stderr, "ImpersonateServer (Authenticated) %d\n", IAuth::AUTH_MORE_DATA));
			return IAuth::AUTH_MORE_DATA;
		}
		...
		{ // instance scope
			...
			if (!instance->userDb)
				instance->userDb = FB_NEW UserDatabase(instance->userDbName);

			staff_name staffname;		// user name buffer
			impersonateUser.copyTo(staffname, sizeof(staffname));
			staff_record staff_block;	// user record
			memset(&staff_block, 0, sizeof staff_block);

			found = instance->userDb->lookup(staffname, &staff_block);
			fb_utils::copy_terminate(impersonateStaffId, staff_block.login, 20 + 1);
		}
		if (!found)
		{
			HANDSHAKE_DEBUG(fprintf(stderr, "ImpersonateServer (badImpersonate) %d\n", IAuth::AUTH_CONTINUE));
			return IAuth::AUTH_CONTINUE;
		}

		FbLocalStatus s;
		authBlock->add(&s, impersonateStaffId);
		check(&s);
		authBlock->setDb(&s, secureDbName.c_str());
		check(&s);
	...
	catch (const Exception& ex)
	{
		...
		HANDSHAKE_DEBUG(fprintf(stderr, "ImpersonateServer: exception status:\n"));
...
namespace {
	SimpleFactory<ImpersonateServer> factory;
}

void registerImpersonateServer(IPluginManager* iPlugin)
{
	iPlugin->registerPluginFactory(IPluginManager::TYPE_AUTH_SERVER,
		"Impersonate_Auth", &factory);
}
...
extern "C" FB_DLL_EXPORT void FB_PLUGIN_ENTRY_POINT(IMaster* master)
{
	...
	Auth::registerImpersonateServer(PluginManagerInterfacePtr());
